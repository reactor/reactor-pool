...
final class PartitionedPool<KEY, POOLABLE> extends ConfigurablePool<POOLABLE>
		implements Function<KEY, InstrumentedPool<POOLABLE>>, Runnable {

	final BiFunction<? super KEY, ? super Runnable, ? extends InstrumentedPool<POOLABLE>> poolFactory;
	...
	PartitionedPool(PoolConfiguration<POOLABLE> poolConfig,
			BiFunction<? super KEY, ? super Runnable, ? extends InstrumentedPool<POOLABLE>> poolFactory,
...
//		this.pending = Queues.get(poolConfig.maxPending).get();
		...
		new MonoAcquire<>(this, Duration.ZERO)
				.repeat(poolConfig.initialSize - 1)
				.blockLast();

//		poolConfig.allocationStrategy.returnPermits(poolConfig.initialSize);
	}
	...
	public InstrumentedPool<POOLABLE> apply(KEY key) {
		InstrumentedPool<POOLABLE> pool = poolFactory.apply(key, this);
...
//									new PartitionAllocationStrategy(releaseCallback),
									poolConfig.maxPending,
									p -> Mono.fromDirect(poolConfig.releaseHandler.apply(p)).doAfterTerminate(signalCallback),
//									p -> Mono.fromDirect(poolConfig.destroyHandler.apply(p)).doAfterTerminate(signalCallback),
									poolConfig.destroyHandler,
									poolConfig.evictionPredicate,
									poolConfig.acquisitionScheduler,
									poolConfig.metricsRecorder,
